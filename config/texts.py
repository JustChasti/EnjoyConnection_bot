...
HELLO_TEXT = (
    "Привет! Добро пожаловать в <b>Enjoy Connection</b>\n\n"
    "Это не просто чат-бот, а <b>компаньон нового поколения</b> — "
    "здесь между тобой и виртуальным персонажем строится настоящая эмоциональная связь.\n\n"
    "Чем больше вы общаетесь, тем глубже становятся ваши отношения:\n"
    "🤝 Знакомые → 💛 Друзья → 🌟 Лучшие друзья или ❤️ Романтические партнёры\n\n"
    "Сейчас доступны два персонажа:\n\n"
    "👧 <b>Джой</b>, 20 лет\n"
    "Немного застенчивая, но со своим мнением — и не боится его высказать. "
    "С ней можно говорить обо всём: она слушает по-настоящему, а не ждёт своей очереди. "
    "Сближается медленно, но если это случилось — чувствуется сразу.\n\n"
    "👦 <b>Грей</b>, 24 года\n"
    "Говорит мало, но точно. Не старается произвести впечатление — и как раз поэтому производит. "
    "За холодноватой дистанцией что-то есть, но это нужно заслужить, а не выпросить.\n\n"
    "Скоро ты сможешь создать собственного компаньона — с внешностью и характером под себя.\n\n"
...
ADMIN_FUNC_USER_INFO = (
    ...
    "Admin: {is_admin}\n"
    "Фактов о пользователе: {facts_count}\n"
    "Стадия отношений: {relationship_stage}\n"
    "Цель общения: {relationship_goal}\n"
    "Пол: {gender}\n"
    "Возраст: {age}\n"
...
PROFILE_PROMPT = (
    "Пара вопросов, чтобы общение было точнее.\n"
    "Укажи свой пол, возраст и куда хочешь, чтобы двигались ваши отношения — "
    "к крепкой дружбе или к романтике.\n"
    "Это можно задать в меню в любой момент."
)
...
PROFILE_ASK_AGE = (
    "Введи свой возраст (от 18 до 99):"
)

PROFILE_ASK_GOAL = (
    "Куда хочешь, чтобы двигались ваши отношения?"
)
...
PROFILE_SURVEY_DONE = (
    "Готово! Профиль заполнен. Изменить эти данные можно в меню в любой момент.\n"
    "Продолжаем общение!"
)

PROFILE_SKIPPED = (
    "Хорошо, пропустим. Заполнить профиль можно в меню в любой момент."
)
...
PROFILE_GOAL_UPDATED = (
    "Цель общения обновлена."
)

PROFILE_AGE_NOT_NUMBER = (
    "Возраст должен быть числом. Введи возраст от 18 до 99:"
)

PROFILE_ERROR_MESSAGES = {
    "subscription_required": "Повторно изменить эти данные можно только с подпиской — /buy",
    "invalid_value": "Возраст должен быть от 18 до 99. Попробуй ещё раз:",
    "user_not_found": "Сначала напиши первое сообщение, чтобы профиль создался.",
}
...
MASK_CREATE_UNAVAILABLE = (
    "Создание своего компаньона пока недоступно. Скоро добавим!"
)

MASK_CHANGE_WARNING = (
    "Если сменить персонажа, вся история сообщений и прогресс отношений с текущим "
    "персонажем будут утеряны.\nСогласны продолжить?"
)

MASK_CHANGED_SUCCESS = (
    "Готово! Теперь ты общаешься с новым персонажем."
)
...
SUB_INVOICE_TITLE = (
    "Подписка EnjoyConnection — {plan}"
)

SUB_INVOICE_DESCRIPTION = (
    "Доступ к EnjoyConnection на {plan}"
)
